package com.as3joelib.utils {
	public class ObjectUtil {
		...
		public static function pr(obj:*, level:int = 0, output:String = ''):* {
			if(level == 0) output = '('+ ObjectUtil.typeOf(obj) +') {\n';
			...
			for(var child:* in obj) {
				...
				if(ObjectUtil.count(obj[child]) == 0) output += obj[child];
				
				var childOutput:String = '';
				if(typeof obj[child] != 'xml') {
					childOutput = ObjectUtil.pr(obj[child], level + 1);
				}
				if(childOutput != '') {
					output += '{\n'+ childOutput + tabs +'}';
				}
		...
		public static function count(obj:Object):uint {
			if(ObjectUtil.typeOf(obj) == 'array') return obj.length;
			else {
				var len:uint = 0;
				for(var item:* in obj) {
